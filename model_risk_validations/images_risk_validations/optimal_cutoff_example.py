...
def calculate_comprehensive_risk_metrics(df, 
    ...
    print("Calculating comprehensive risk metrics for Test set...")
    ...
    if real_loan_amount_col in test_df.columns:
        ...
        test_df['loan_amount'] = 1000  # Default value
    ...
    for decile_col in decile_columns:
        ...
        # Calculate loss savings and revenue opportunity cost for each loan
        # These will be used in cutoff policy analysis
        for cutoff in range(1, 11):
            # For each potential cutoff, calculate metrics
            # Loans with decile > cutoff would be rejected
            rejected_mask = model_df['decile'] > cutoff
            
            # Loss savings = expected loss of rejected loans (what we avoid by rejecting)
            model_df[f'loss_savings_{cutoff}'] = np.where(rejected_mask, 
                                                         model_df['annualized_expected_loss'], 
                                                         0)
            
            # Revenue opportunity cost = interest revenue of rejected loans (what we give up by rejecting)
            model_df[f'revenue_opportunity_cost_{cutoff}'] = np.where(rejected_mask, 
                                                                     model_df['annual_interest_revenue'], 
                                                                     0)
            
            # Net savings = loss savings - revenue opportunity cost
            model_df[f'net_savings_{cutoff}'] = model_df[f'loss_savings_{cutoff}'] - model_df[f'revenue_opportunity_cost_{cutoff}']
        
        # Group by decile and calculate aggregated metrics
        grouped = model_df.groupby('decile').agg(
            ...
            annualized_expected_loss=('annualized_expected_loss', 'sum'),
            annual_interest_revenue=('annual_interest_revenue', 'sum'),
            total_interest_revenue=('total_interest_revenue', 'sum'),
            avg_lgd=('lgd', 'mean'),
            weighted_lgd=lambda x: (x['lgd'] * x['loan_amount']).sum() / x['loan_amount'].sum() if x['loan_amount'].sum() > 0 else 0,
            avg_interest_rate=('interest_rate', 'mean'),
            weighted_interest_rate=lambda x: (x['interest_rate'] * x['loan_amount']).sum() / x['loan_amount'].sum() if x['loan_amount'].sum() > 0 else 0,
            avg_term=('term', 'mean'),
            weighted_term=lambda x: (x['term'] * x['loan_amount']).sum() / x['loan_amount'].sum() if x['loan_amount'].sum() > 0 else 0
        ).reset_index()
        
        # Calculate additional metrics
        grouped['non_defaults'] = grouped['total_loans'] - grouped['defaults']
        grouped['defaulted_exposure'] = grouped.apply(
            lambda row: model_df[(model_df['decile'] == row['decile']) & (model_df['default'] == 1)]['loan_amount'].sum(), 
            axis=1
        )
        grouped['loss_rate'] = (grouped['defaulted_exposure'] / grouped['total_exposure'] * 100).fillna(0)
        
        # Calculate portfolio percentages
        total_test_loans = len(model_df)
        grouped['pct_of_loans'] = (grouped['total_loans'] / total_test_loans) * 100
        grouped['pct_of_defaults'] = (grouped['defaults'] / total_defaults) * 100 if total_defaults > 0 else 0
        grouped['pct_of_exposure'] = (grouped['total_exposure'] / total_exposure) * 100 if total_exposure > 0 else 0
        
        # Calculate lift
        grouped['lift'] = grouped['default_rate'] / overall_default_rate if overall_default_rate > 0 else 0
        
        # Add cumulative metrics
        grouped = grouped.sort_values('decile')
        grouped['cumulative_defaults_pct'] = grouped['pct_of_defaults'].cumsum()
        grouped['cumulative_exposure_pct'] = grouped['pct_of_exposure'].cumsum()
        grouped['cumulative_expected_loss'] = grouped['expected_loss'].cumsum()
        
        # Add model name
        grouped['model'] = model_name
        
        # Add cutoff policy metrics to the grouped data
        for cutoff in range(1, 11):
            # Sum the metrics for all loans with decile > cutoff
            loss_savings = model_df[model_df['decile'] > cutoff][f'loss_savings_{cutoff}'].sum()
            revenue_opportunity_cost = model_df[model_df['decile'] > cutoff][f'revenue_opportunity_cost_{cutoff}'].sum()
            net_savings = model_df[model_df['decile'] > cutoff][f'net_savings_{cutoff}'].sum()
            
            # Add to the grouped data
            grouped[f'loss_savings_cutoff_{cutoff}'] = loss_savings
            grouped[f'revenue_opportunity_cost_cutoff_{cutoff}'] = revenue_opportunity_cost
            grouped[f'net_savings_cutoff_{cutoff}'] = net_savings
        
        # Store both the aggregated results and the loan-level data
        results[model_name] = {
            'decile_metrics': grouped,
...
def create_risk_validation_visualizations(risk_results):
    ...
    for decile in range(1, 11):
        row = [f"{decile:<10}"] 
        for model, df in risk_results.items():
            model_decile = df[df['decile'] == decile]
    ...
    for decile in range(1, 11):
        ...
        for model, df in risk_results.items():
            model_decile = df[df['decile'] == decile]
            loss_rate = model_decile['loss_rate'].values[0] * 100 if len(model_decile) > 0 else 0
    ...
    for model, df in risk_results.items():
        fig1.add_trace(go.Scatter(
            x=df['decile'], y=df['default_rate'] * 100,
    ...
    for model, df in risk_results.items():
        fig1.add_trace(go.Scatter(
            x=df['decile'], y=df['loss_rate'] * 100,
            mode='lines+markers', name=f'{model}',
            line=dict(color=colors.get(model, '#000000'), width=2),
    ...
    for model, df in risk_results.items():
        fig1.add_trace(go.Bar(
            x=df['decile'], y=df['expected_loss'],
    ...
    concentration_data = {}
    for model, df in risk_results.items():
        top1 = df['cumulative_defaults_pct'].iloc[0]  # Top 1 decile
        top2 = df['cumulative_defaults_pct'].iloc[1]  # Top 2 deciles  
        top3 = df['cumulative_defaults_pct'].iloc[2]  # Top 3 deciles
...
def create_advanced_xgboost_analysis(risk_results):
    ...
    for pct in portfolio_pcts:
        ...
        for model, df in risk_results.items():
            ...
            closest_idx = (df['cumulative_exposure_pct'] - pct).abs().idxmin()
            
            # Calcular defaults acumulados si no existe la columna
            if 'cumulative_defaults' not in df.columns:
                # Ordenar por deciles (de mejor a peor)
                sorted_df = df.sort_values('decile')
                ...
                default_capture = df.loc[closest_idx, 'cumulative_defaults'] / df['defaults'].sum() * 100
    ...
    for pct in portfolio_pcts:
        ...
        for model, df in risk_results.items():
            ...
            if 'cumulative_expected_loss' not in df.columns:
                # Ordenar por deciles (de mejor a peor)
                sorted_df = df.sort_values('decile')
                ...
                loss_capture = df.loc[closest_idx, 'cumulative_expected_loss'] / df['expected_loss'].sum() * 100
    ...
    for model, df in risk_results.items():
        ...
        portfolio_pct = df['pct_of_loans'].cumsum()
        defaults_pct = df['cumulative_defaults_pct']
    ...
    for model, df in risk_results.items():
        ...
        top_3_defaults = df[df['decile'].isin([8, 9, 10])]['pct_of_defaults'].sum()
        top_2_defaults = df[df['decile'].isin([9, 10])]['pct_of_defaults'].sum()
        top_1_defaults = df[df['decile'] == 10]['pct_of_defaults'].iloc[0]
    ...
    baseline_results = risk_results['Baseline']
    economic_data = []
...
def analyze_cutoff_policy(risk_results, cutoff_decile=4, interest_rate=0.15, interest_rate_col=None, lgd=None, lgd_col=None, term_col=None, default_term=36):
    ...
    for model, df in risk_results.items():
        ...
        if term_col and term_col in df.columns:
            ...
            if not pd.api.types.is_numeric_dtype(rejected_terms):
                # Extract numeric part from strings like '36 months'
        ...
        if  interest_rate_col  in rejected_df.columns and interest_rate_col is not None:
            ...
            rejected_interest = ((rejected_df['real_loan_amount'] * interest_rates) * (12 / rejected_terms)).sum()
        else:
            # Use fixed interest rate if no loan-specific rates (annualized)
            rejected_interest = (rejected_amount * interest_rate * (12 / rejected_terms.mean()))
        
        # Apply LGD to expected loss if provided
        if lgd_col in rejected_df.columns and lgd_col is not None:
            # Use loan-specific LGD values if available
            if isinstance(rejected_df[lgd_col].iloc[0], str):
                # Convert percentage strings to float (e.g., '60%' to 0.6)
            ...
            rejected_expected_loss = rejected_expected_loss * lgd_values.mean()
        ...
        if term_col and term_col in df.columns and term_col is not None:
